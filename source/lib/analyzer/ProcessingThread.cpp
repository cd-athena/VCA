...
bool ProcessingThread::computeWeightedDCTEnergy(const vca_param &cfg,
                                                vca_frame *frame,
                                                vca_frame_results *m_texture)
{
    ...
    if (frame->info.bitDepth > 8)
    {
        ...
        log(cfg, LogLevel::Error, "16 bit not implemented yet.");
        return false;
    }
    ...
    //                 {
    //                     weightedDCTCoeff[p * 32 + q] = (int32_t)(weightFactorMatrix[p * 32 + q]
    //                                                              * abs(dctCoeff[p * 32 + q]));
    ...
    //             }
    //             /* Third 32x32 block */
    //             primitives.cu[3].copy_ss(srcCoeff1,
    //                                      32,
    //                                      srcCoeff + (32 * maxBlockSize),
    //                                      maxBlockSize);
    //             primitives.cu[3].dct(srcCoeff1, dctCoeff, 32);
    //             for (uint32_t p = 0; p < 32; p++)
    //             {
    ...
    //                 for (uint32_t q = 0; q < maxBlockSize; q++)
    //                 {
    //                     weightedDCTCoeff[p * maxBlockSize + q] = (int32_t)(
    //                         weightFactorMatrix[p * maxBlockSize + q]
    //                         * abs(dctCoeff[p * maxBlockSize + q]));
    ...
    //         m_texture->m_ctuRelativeEnergy[p + q * numCuInWidth]
    //             = (double) m_texture->m_ctuAbsoluteEnergy[p + q * numCuInWidth]
    //               / m_texture->m_avgEnergy;
    //     }
    // }
    return true;
}

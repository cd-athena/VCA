...
ThreadPool* ThreadPool::allocThreadPools(vca_param* p, int& numPools, bool isThreadsReserved)
{
    ...
    int numNumaNodes = VCA_MIN(getNumaNodeCount(), MAX_NODE_NUM);
    ...
    if (bNumaSupport)
    {
        ...
        for (int i = 0; i < numNumaNodes; i++)
        {
            ...
            if (!ret)
                ...
                vca_log(p, VCA_LOG_ERROR, "Failed to genrate CPU mask\n");
        }
    ...
    if (bNumaSupport && p->logLevel >= VCA_LOG_DEBUG)
    for (int i = 0; i < numNumaNodes; i++)
        vca_log(p, VCA_LOG_DEBUG, "detected NUMA node %d with %d logical cores\n", i, cpusPerNode[i]);
    ...
    if (!p->numaPools || (strcmp(p->numaPools, "NULL") == 0 || strcmp(p->numaPools, "*") == 0 || strcmp(p->numaPools, "") == 0))
    {
         ...
         param_parse(p, "pools", poolString);
     }
#endif
    if (p->numaPools && *p->numaPools)
    {
        ...
        for (int i = 0; i < numNumaNodes; i++)
        {
            ...
            else
            {
                ...
                {
                    threadsPerPool[i] = VCA_MIN(count, cpusPerNode[i]);
                    nodeMaskPerPool[i] = ((uint64_t)1 << i);
                }
                else                                 // new logic: exactly 'count' threads on all NUMAs
                {
                    threadsPerPool[numNumaNodes] = VCA_MIN(count, numNumaNodes * MAX_POOL_THREADS);
    ...
    {
        if (bNumaSupport)
            vca_log(p, VCA_LOG_DEBUG, "NUMA node %d may use %d logical cores\n", i, cpusPerNode[i]);
        if (threadsPerPool[i])
        {
    ...
    if (!isThreadsReserved)
    {
        if (!numPools)
        {
            vca_log(p, VCA_LOG_DEBUG, "No pool thread available. Deciding frame-threads based on detected CPU threads\n");
    ...
    {
        vca_log(p, VCA_LOG_DEBUG, "Reducing number of thread pools for frame thread count\n");
        numPools = VCA_MAX(p->frameNumThreads / 2, 1);
    }
    if (isThreadsReserved)
        numPools = 1;
    ...
    if (pools)
    {
        ...
        for (int i = 0; i < numPools; i++)
        {
            while (!threadsPerPool[node])
                node++;
            int numThreads = VCA_MIN(MAX_POOL_THREADS, threadsPerPool[node]);
            ...
            if (numNumaNodes > 1)
            {
                ...
                vca_log(p, VCA_LOG_INFO, "Thread pool %d using %d threads on numa nodes %s\n", i, numThreads, nodesstr + 1);
                delete[] nodesstr;
            }
            else
                vca_log(p, VCA_LOG_INFO, "Thread pool created using %d threads\n", numThreads);
...
{
    VCA_CHECK(numThreads <= MAX_POOL_THREADS, "a single thread pool cannot have more than MAX_POOL_THREADS threads\n");
    ...
    if (numa_available() >= 0)
    {
        ...
        if (nodemask)
        {
            *(nodemask->maskp) = nodeMask;
            ...
            vca_log(NULL, VCA_LOG_ERROR, "unable to get NUMA node mask for %lx\n", nodeMask);
    }
#else
    (void)nodeMask;
#endif
    ...
    m_workers = VCA_MALLOC(WorkerThread, numThreads);
    /* placement new initialization */
    if (m_workers)
        for (int i = 0; i < numThreads; i++)
            new (m_workers + i)WorkerThread(*this, i);

    m_jpTable = VCA_MALLOC(JobProvider*, maxProviders);
    m_numProviders = 0;
...
void ThreadPool::setThreadNodeAffinity(void *numaMask)
{
    ...
    if (SetThreadGroupAffinity(GetCurrentThread(), affinityPointer, NULL))
        ...
        vca_log(NULL, VCA_LOG_ERROR, "unable to set thread affinity for NUMA node mask\n");
#elif HAVE_LIBNUMA
    if (numa_available() >= 0)
    {
...
void ThreadPool::getFrameThreadsCount(vca_param* p, int cpuCount)
{
    int rows = (p->sourceHeight + p->maxCUSize - 1) >> g_log2Size[p->maxCUSize];
    if (!p->bEnableWavefront)
        p->frameNumThreads = VCA_MIN3(cpuCount, (rows + 1) / 2, VCA_MAX_FRAME_THREADS);
    else if (cpuCount >= 32)

...
public:
    ...
    Event()
    {
        ...
        {
            vca_log(NULL, VCA_LOG_ERROR, "fatal: unable to initialize conditional variable\n");
        }
    }
...
{
    ScopedElapsedTime(int64_t& accum) : accumlatedTime(accum) { startTime = vca_mdate(); }

    ~ScopedElapsedTime() { accumlatedTime += vca_mdate() - startTime; }

...
public:
    ...
    size_t calcualteFrameBytes()
    {
        size_t framesizeBytes = 0;
        auto pixelbytes       = frameInfo.bitDepth > 8 ? 2u : 1u;
        for (int i = 0; i < vca_cli_csps.at(frameInfo.colorspace).planes; i++)
        {
            uint32_t w = frameInfo.width >> vca_cli_csps.at(frameInfo.colorspace).width[i];
            uint32_t h = frameInfo.height >> vca_cli_csps.at(frameInfo.colorspace).height[i];

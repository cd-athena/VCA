...
YUVInput::YUVInput(InputFileInfo& info)
{
    ...
    {
        uint32_t w = width >> vca_cli_csps[colorSpace].width[i];
        uint32_t h = height >> vca_cli_csps[colorSpace].height[i];
        framesize += w * h * pixelbytes;
    }

    if (width == 0 || height == 0 || info.fpsNum == 0 || info.fpsDenom == 0)
    {
        vca_log(NULL, VCA_LOG_ERROR, "yuv: width, height, and FPS must be specified\n");
        return;
    }
    ...
    {
        buf[i] = VCA_MALLOC(char, framesize);
        if (buf[i] == NULL)
        {
            vca_log(NULL, VCA_LOG_ERROR, "yuv: buffer allocation failure, aborting\n");
            threadActive = false;
            return;
        }
    }
...
bool YUVInput::readPicture(vca_picture& pic)
{
    int read = readCount.get();
    int written = writeCount.get();

...
Y4MInput::Y4MInput(InputFileInfo& info)
{
    ...
    if (!strcmp(info.filename, "-"))
    {
        ...
        ifs = vca_fopen(info.filename, "rb");
    if (ifs && !ferror(ifs) && parseHeader())
    {
        int pixelbytes = depth > 8 ? 2 : 1;
        for (int i = 0; i < vca_cli_csps[colorSpace].planes; i++)
        {
            int stride = (width >> vca_cli_csps[colorSpace].width[i]) * pixelbytes;
            framesize += (stride * (height >> vca_cli_csps[colorSpace].height[i]));
        }

        threadActive = true;
        for (int q = 0; q < QUEUE_SIZE; q++)
        {
            buf[q] = VCA_MALLOC(char, framesize);
            if (!buf[q])
            {
                vca_log(NULL, VCA_LOG_ERROR, "y4m: buffer allocation failure, aborting");
                threadActive = false;
                break;
            }
        }
...
bool Y4MInput::parseHeader()
{
        ...
        while (c == ' ')
        {
            ...
            case 'C':
                ...
                else if (csp == ('m'-'0')*1000 + ('o'-'0')*100 + ('n'-'0')*10 + ('o'-'0'))
                {
                    ...
                    colorSpace = (csp == 444) ? VCA_CSP_I444 : (csp == 422) ? VCA_CSP_I422 : VCA_CSP_I420;
    ...
    if (width < MIN_FRAME_WIDTH || width > MAX_FRAME_WIDTH ||
        ...
        colorSpace < VCA_CSP_I400 || colorSpace >= VCA_CSP_COUNT)
        return false;
...
bool Y4MInput::populateFrameQueue()
{
    ...
    {
        if (!feof(ifs))
            vca_log(NULL, VCA_LOG_ERROR, "y4m: frame header missing\n");
        return false;
    }
...
bool Y4MInput::readPicture(vca_picture& pic)
{
    int read = readCount.get();
    int written = writeCount.get();
    ...
    if (read < written)
    {
        ...
        pic.stride[1] = pic.stride[0] >> vca_cli_csps[colorSpace].width[1];
        pic.stride[2] = pic.stride[0] >> vca_cli_csps[colorSpace].width[2];
        pic.planes[0] = buf[read % QUEUE_SIZE];
        pic.planes[1] = (char*)pic.planes[0] + pic.stride[0] * height;
        pic.planes[2] = (char*)pic.planes[1] + pic.stride[1] * (height >> vca_cli_csps[colorSpace].height[1]);
        readCount.incr();
        return true;
    }
    else

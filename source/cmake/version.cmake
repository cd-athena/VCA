...
if(EXISTS ${CMAKE_CURRENT_SOURCE_DIR}/../.hg_archival.txt)
    ...
    endforeach()
    if(DEFINED hg_tag)
        set(VCA_LATEST_TAG ${hg_tag})
    elseif(DEFINED hg_node)
        set(VCA_LATEST_TAG ${hg_latesttag})
        set(VCA_TAG_DISTANCE ${hg_latesttagdistance})
        string(SUBSTRING "${hg_node}" 0 12 VCA_REVISION_ID)
    endif()
elseif(HG_EXECUTABLE AND EXISTS ${CMAKE_CURRENT_SOURCE_DIR}/../.hg)
    ...
    string(SUBSTRING "${VCA_REVISION_ID}" 0 12 VCA_REVISION_ID)

    if(VCA_LATEST_TAG MATCHES "^r")
        string(SUBSTRING ${VCA_LATEST_TAG} 1 -1 VCA_LATEST_TAG)
    endif()
